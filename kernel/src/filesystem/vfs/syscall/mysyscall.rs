...
pub struct MyKernelWriteHandle;

impl Syscall for MyKernelWriteHandle {
    /// Returns the number of arguments expected by the `write` syscall
    fn num_args(&self) -> usize {
        0
    }

    /// Handles the `Kernelwrite` system call
    ///
    /// Just write mysyscall666
    ///
    /// # Arguments
    /// None
    /// # Returns
    /// * `Ok(usize)` - 1 successfully written
    /// * `Err(SystemError)` - Error code if operation fails
    fn handle(&self, args: &[usize], frame: &mut TrapFrame) -> Result<usize, SystemError> {
        log::info!("This is my syscall ,2333syscall");
        return  Ok(6666);
        // let fd = Self::fd(args);
...
//     }
// }
pub const MYSYSCALL:usize = 2333;
syscall_table_macros::declare_syscall!(MYSYSCALL, MyKernelWriteHandle);
